...
def main():
    ...
    if args.config:
        ...
        if not config_path.exists():
            # Fall back to project root (handles running from any subdirectory)
            config_path = Path(__file__).resolve().parents[3] / Path(args.config).name
        if not config_path.exists():
    ...
    if config['interface_manager']['docker']:
        ...
        interface_manager_url = config.get("interface_manager", {}).get("base_url_local", "http://localhost:8000")
